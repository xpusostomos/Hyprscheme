# exec spawns (side-effect file), raw exec, exec-with-rules
rm -f /tmp/hs-test-exec
$SCHEME '(hl-exec-shell! "touch /tmp/hs-test-exec")' | grep -qE '^[0-9]+$' || { echo "exec pid failed"; exit 1; }
...
echo "exec-raw gave: $out"
[[ "$out" == "#t" ]] || { echo "exec-raw failed"; exit 1; }
for _ in $(seq 1 20); do [[ -f /tmp/hs-test-raw ]] && break; sleep 0.2; done
[[ -f /tmp/hs-test-raw ]] || { echo "raw side effect missing"; exit 1; }

$SCHEME '(hl-exec-shell-with-rules! "[float] foot -a rules-test")' | grep -q '#t' || { echo "exec-with-rules failed"; exit 1; }
sleep 1
