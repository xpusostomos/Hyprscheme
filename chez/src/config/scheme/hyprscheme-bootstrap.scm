...
(defun hl-layout-add! (name . spec) "Register the custom layout NAME.
...
(defun hl-submap (name fn . reset) "Run FN to define a submap: binds
...
(defun hl-submap-activate! (name) "Switch the active submap to NAME
(\"\" or \"reset\" returns to the default map)."
  (= 0 (c-hl-enter-submap name)))

(defun hl-submap-exit! () "Leave the active submap and return to the
default map."
  (= 0 (c-hl-enter-submap "")))
...
(defun hl-plist-get (plist key . default) "Read KEY out of PLIST (the
...
(defun hl-bind-add! (tokens thunk . opts) "Register a keybind: TOKENS
is the key token list — modifiers first, key last — normally built by
(hl-kbd \"C-M-a\"), (hl-key \"SUPER+A\") or written out literally
'\"SUPER\" \"Q\" as a quoted list; a modless key is still a list.
THUNK is the action; OPTS may carry 'description and 'devices. Returns
the bind handle; remove with (hl-unbind! B). There is no two-string
shorthand in the core API — define your own wrapper on top if you want
one."
...
(defun hl-kbd (spec) "Parse an emacs-style key specification string
...
(defun hl-key (spec) "Parse a hyprland-style key specification string
into a LIST of key tokens: \"SUPER+SHIFT+Q\" →
(\"SUPER\" \"SHIFT\" \"Q\")."
  (map hl--trim (hl--split-string spec #\+)))

(defun hl-after (ms thunk) "Run THUNK once after MS milliseconds.
...
(defun hl-repeat (ms thunk) "Run THUNK every MS milliseconds until
...
(defun hl-active-title () "Title of the focused window, or #f when no
window has focus."
  (c-hl-active-title))
...
(defun hl-window-center! (w)
  "Center WINDOW."
  (= 0 (c-hl-window-center (hl--wid w))))

(defun hl-window-size-set! (w width height . opt)
...
(defun hl-window-deny-from-group-set! (w . on?)
...
(defun hl-workspace-special-set! (ws . on?)
...
(defun hl-monitor-workspace-special-set! (mon ws)
...
(defun hl-cursor-move! (x y)
  "Move the cursor to (X, Y)."
  (= 0 (c-hl-cursor-move (exact->inexact x) (exact->inexact y))))

(defun hl-cursor-move-to-corner! (w corner)
...
(defun hl-window-send-key-state! (mods key state . w)
...
(defun hl-clear-crashed-lockscreen! ()
...
(defun hl-exec-scheduled-prop-refresh-immediately ()
...
(defun hl-device-add! (name . fields)
  "Configure a device: NAME plus a spread plist of fields. Write-only
   (no device read side, no per-field unset); validated against the
   field table and applied to the live device."
...
(defun hl-window-rule-add! (name . spec)
  "Add a window rule and return its rule handle. NAME (#f = anonymous) +
   a rule spec plist: 'match maps properties to values (class, title,
   initial_class, floating, workspace, ...), 'enabled toggles the rule,
   and every other key is an effect (float, fullscreen, monitor,
   workspace, opacity, ...). The window-rule page has the full lists.
   Anonymous rules are re-created on each config reload; named rules are
   reused."
...
(defun hl-layer-rule-add! (name . spec)
  "Add a layer rule: the same rule-spec shape as hl-window-rule-add!,
...
(defun hl-rule-enabled-set! (rule . on?)
...
(defun hl-group-add! (g window . index)
...
(defun hl-group-remove! (g window)
...
(defun hl-workspace-rule-add! (ws . spec)
  "Add a workspace rule for WS (a selector or handle):
   (hl-workspace-rule-add! \"3\"'monitor \"DP-1\"'layout \"master\").
   Fields: monitor default persistent gaps_in gaps_out float_gaps
   border_size no_border no_rounding decorate no_shadow on_created_empty
   default_name layout animation layout_opts."
  (let ((enabled (hl--plist-get spec 'enabled #t)))
...
(defun hl-layers filters
  "The layer surfaces, as a list of layer handles. Optional filters:
   'monitor MON, 'namespace \"ns\", or both combined."
...
(defun hl-notification-add! fields
  "Create a live notification and return its handle. Fields: 'text,
   'timeout (required), optionally 'icon, 'color, 'font-size. Pausing
   freezes the timeout timer (the bubble stays until dismissed); expired
   handles read #f and mutate as no-ops."
...
(defun hl-notification-text-set! (n text)
...
(defun hl-notification-timeout-set! (n ms)
...
(defun hl-notification-color-set! (n color)
...
(defun hl-notification-icon-set! (n icon)
...
(defun hl-notification-font-size-set! (n size)
...
(defun hl-notification-paused-set! (n . on?)
...
(defun hl-timer-enabled? (t)
  "#t when the timer T is enabled."
  (= 1 (c-hl-timer-enabled t)))
(defun hl-timer-set-timeout (t ms)
...
(defun hl-make-float-gesture mode
...
(defun hl-make-fullscreen-gesture mode
...
(defun hl-make-special-workspace-gesture (name)
...
(defun hl-make-cursor-zoom-gesture (zoom . mode)
...
(defun hl-make-custom-gesture fields
...
(defun hl-gesture-add! fields
  "Register a gesture: (hl-gesture-add! 'fingers N 'direction \"dir\"
   'action ACTION ['mods (hl-key \"SUPER\")] ['scale 1.0]
   ['disable-inhibit #t]). Fingers and direction are required; the
   optional fields describe the gesture INPUT, independent of the
   action. ACTION is an hl-gesture-action built by the hl-make-*-gesture
   constructors - built-in and custom actions are indistinguishable from
   the caller's side, and a recipe is a pure value (registering it under
   two specs constructs two independent gestures). Returns an hl-gesture
   handle for hl-gesture-remove! - removal matches on the registration
   spec, never on the action."
...
(defun hl-monitor-rule-add! (output . fields)
  "Add a monitor rule for OUTPUT (name or desc: selector), e.g.
   (hl-monitor-rule-add! \"DP-1\"'((mode . \"preferred\") (scale .
   \"1.6\") (position . \"0x0\") (transform . 0) (bitdepth . 10) (vrr .
   1) (reserved . '((top . 60))))). String fields: mode position scale
   mirror cm icc sdr_eotf. Numeric fields: transform bitdepth vrr
   supports_wide_color supports_hdr sdrbrightness sdrsaturation
   sdr_min_luminance sdr_max_luminance min_luminance max_luminance
   max_avg_luminance. Gap fields (plist): reserved / reserved_area.
   Bool: disabled."
...
(defun hl-curve-add! (name type . vals)
...
(defun hl-animation-add! (leaf . opts)
  "Configure an animation: (hl-animation-add! \"windowsIn\"'enabled #t
   'speed 8 'curve \"mycurve\"'style \"popin 80%\"). Speed defaults to
   8; declare curves with hl-curve-add! first (or use builtins like
   \"default\")."
...
(defun hl-permission-add! (binary type mode)
...
(defun hl-window-fullscreen-set! (w . on?)
...
(defun hl-window-maximized-set! (w . on?)
...
(defun hl-window-fullscreen-state (w internal client . layout-aware)
...
(defun hl-window-open-notification-add! (handler)
...
(defun hl-window-close-notification-add! (handler)
...
(defun hl-window-title-notification-add! (handler)
...
(defun hl-window-class-notification-add! (handler)
...
(defun hl-window-urgent-notification-add! (handler)
...
(defun hl-window-pin-notification-add! (handler)
...
(defun hl-window-fullscreen-notification-add! (handler)
...
(defun hl-window-move-to-workspace-notification-add! (handler)
...
(defun hl-window-active-notification-add! (handler)
...
(defun hl-window-minimize-notification-add! (handler)
...
(defun hl-window-open-early-notification-add! (handler)
...
(defun hl-window-kill-notification-add! (handler)
...
(defun hl-window-bell-notification-add! (handler)
...
(defun hl-window-update-rules-notification-add! (handler)
...
(defun hl-start-notification-add! (handler)
...
(defun hl-shutdown-notification-add! (handler)
...
(defun hl-config-reloaded-notification-add! (handler)
...
(defun hl-config-unload-notification-add! (handler)
...
(defun hl-window-destroy-notification-add! (handler)
...
(defun hl-layer-open-notification-add! (handler)
...
(defun hl-layer-close-notification-add! (handler)
...
(defun hl-keyboard-key-notification-add! (handler)
...
(defun hl-screenshare-state-notification-add! (handler)
...
(defun hl-config-props-refreshed-notification-add! (handler)
...
(defun hl-submap-notification-add! (handler)
...
;; ---- workspace events ---------------------------------------------------------
(defun hl-workspace-active-notification-add! (handler)
...
(defun hl-workspace-created-notification-add! (handler)
...
(defun hl-workspace-removed-notification-add! (handler)
...
(defun hl-workspace-special-active-notification-add! (handler)
...
(defun hl-workspace-move-to-monitor-notification-add! (handler)
...
(defun hl-monitor-added-notification-add! (handler)
...
(defun hl-monitor-removed-notification-add! (handler)
...
(defun hl-monitor-focused-notification-add! (handler)
...
(defun hl-monitor-layout-changed-notification-add! (handler)
...
(defun hl-state-set! (k v) "Cross-reload state: store VALUE under KEY.
...
(defun hl-state-ref (k . default) "Cross-reload state: KEY's value, or
...
(defun hl-state-keys () "Cross-reload state: the list of stored keys."
  (let loop ((l hl--state) (acc '()))
    (if (null? l) (reverse acc) (loop (cddr l) (cons (car l) acc)))))

(defun hl-state-remove! (k) "Remove KEY from the cross-reload state;
