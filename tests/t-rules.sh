...
$SCHEME '(hl-window-rule-add! "float-check" #:match (quote (class "^rule-check$")) #:float #t)' >/dev/null || { echo "rule registration failed"; exit 1; }
...
$SCHEME '(define rule-check2 (hl-window-rule-add! "float-check2" #:match (quote (class "^rule-check2$")) #:float #t))' >/dev/null
$SCHEME '(hl-rule-enabled-set! rule-check2 #:on? #f)' >/dev/null
