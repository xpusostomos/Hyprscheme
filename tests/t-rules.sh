...
$SCHEME '(hl-window-rule "float-check" (quote ((match . ((class . "^rule-check$"))) (float . #t))))' >/dev/null || { echo "rule registration failed"; exit 1; }
...
$SCHEME '(define rule-check2 (hl-window-rule "float-check2" (quote ((match . ((class . "^rule-check2$"))) (float . #t)))))' >/dev/null
